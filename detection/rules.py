RULES = {
    ...
    "external_ip": {
        ...
        "check": lambda row: not str(row.get("ip", "")).startswith(("192.", "10.", "172.")),
    },
    ...
    "injected_process_network": {
        ...
        "check": lambda row: str(row.get("process", "")).lower()
        in ["notepad.exe", "mspaint.exe"]
    ...
    "suspicious_user_agent": {
        ...
        "check": lambda row: any(
            candidate in str(row.get("user_agent", "")).lower()
            for candidate in ["python-requests", "curl", "winhttp"]
        ),
    },
}
